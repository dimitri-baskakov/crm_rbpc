        ...
        <template #item.name="{ item }">
          <div 
            ...
            >
              <div class="mr-3">
                <a
                  @click.stop="openCategoryProductDialog(item.id)"
...
export default {
  ...
  computed: {
    ...
    items() {
      this.selectedProducts = this.offerProducts
    ...
    selectedHeadersData: {
      ...
      set: async function (newValue) {
        await this.$store.dispatch('saveSetting', {
          selectedHeadersCategoryProductsOffer: JSON.stringify(newValue),
        })
    ...
    selectedHeaders () {
      let selectedHeadersCategoryProductsOffer = this.settings && this.settings.length && this.settings.find(s => s.name == 'selectedHeadersCategoryProductsOffer')
      selectedHeadersCategoryProductsOffer = selectedHeadersCategoryProductsOffer && JSON.parse(selectedHeadersCategoryProductsOffer.value)
      return selectedHeadersCategoryProductsOffer
    },
  },
  ...
  methods: {
    ...
    toggleSelectAll(selected) {
      ...
      if (selected.value) {
        ...
        this.selectedProducts.forEach(group => {
          this.addOfferProduct(group)
        })
      }
      this.$eventHandle.$emit('updateOfferCost')
    },

    selected(group) {
      if (!!this.selectedProducts.find(gr => gr.id === group.id)) {
        this.selectedProducts = this.selectedProducts.filter(gr => gr.id !== group.id)
        this.removeOfferProduct(group)
      } else {
    ...
    addOfferProduct(product) {
      product.manufacturer_id = Object.keys(this.manufacturerNames).find(key => this.manufacturerNames[key] == product.manufacturer)
      product.qty = 1
      ...
      if (!offerProducts.some(sp => sp.id === product.id)) {
    ...
    removeOfferProduct(product) {
      let offerProducts = this.offerProducts;
      offerProducts = offerProducts.filter(op => op.id !== product.id)
      this.$store.commit('setOfferProducts', offerProducts)
    },
