...
export default {
  ...
  created() {
    this.initialize();
    this.selectedGroups = this.offerProducts.map(p => p.group)
  ...
  methods: {
    ...
    removeOfferProduct(group) {
      let product = group.product
      let offerProducts = this.offerProducts;
      offerProducts = offerProducts.filter(pr => pr.id !== product.id)
      this.$store.commit('setOfferProducts', offerProducts)
    },
