  ...
  <div class="" style="">
    ...
    >
      ...
      <template #item.status="{ item }">
        <div class="font-12" :style="{'color': item.status_color}">
          {{ item.status }}<br/>
...
export default {
  name: "PurchaseCategoryProducts",
  components: {
    GroupProductForm: () => import("../../components/category/GroupProductForm"),
    LightBox: () => import("@/views/components/core/LightBox"), SortIndicator: () => import("@/views/components/core/SortIndicator"),
  },
  data () {
    return {
      _timerId: 0,
  ...
  props: ['dateNumber', 'warranties', 'productData', 'searchText'],
    ...
    selectedHeadersData: {
      ...
      set: async function (newValue) {
        await this.$store.dispatch('saveSetting', {
          selectedHeadersCategoryProductsPurchase: JSON.stringify(newValue),
        })
        await this.$store.dispatch('getSettings', {settingsName: 'selectedHeadersCategoryProductsPurchase'})
      }
    },

    selectedHeaders () {
      let selectedHeadersCategoryProductsPurchase = this.settings && this.settings.length && this.settings.find(s => s.name == 'selectedHeadersCategoryProductsPurchase')
      selectedHeadersCategoryProductsPurchase = selectedHeadersCategoryProductsPurchase && JSON.parse(selectedHeadersCategoryProductsPurchase.value)
      return selectedHeadersCategoryProductsPurchase
    },
  },

  created() {
    this.initialize();
    this.selectedProducts = this.purchaseProducts
    this.$eventHandle.$on('sync-selected-products', (products) => {
  ...
  methods: {
    initialize() {
      this.headersMap = [
        ...
        {text: this.$t('status'), value: 'status', align: 'center', sortable: false, class: 'caption', cellClass: 'text-h4', width: 90},
      ];
    },

    clickRow(item) {
      if (!!this.selectedProducts.find(p => item.id === p.id)) {
        this.selectedProducts = this.selectedProducts.filter(p => item.id !== p.id)
        this.$store.commit('setPurchaseProducts', this.selectedProducts)
      } else {
        this.addPurchaseProduct(item)
      }
      this.$eventHandle.$emit('updatePurchaseCost')
    },
    itemSelected(value) {
      if (!!this.selectedProducts.find(p => value.item.id === p.id)) {
        this.selectedProducts = this.selectedProducts.filter(p => value.item.id !== p.id)
        this.$store.commit('setPurchaseProducts', this.selectedProducts)
      } else {
        this.addPurchaseProduct(value.item)
      }
      this.$eventHandle.$emit('updatePurchaseCost')
    },
    ...
    toggleSelectAll(selected) {
      if (selected.value) {
        for (let product of selected.items){
          if (!this.selectedProducts.find(prod => prod.id === product.id)) {
    ...
    addPurchaseProduct(product) {
      ...
      product.purchase_producer_price = product.producer_price
      product.purchase_marginal_price = product.marginal_price
      ...
      if (!!product.warranty) {
        if (!this.warranties.find(w => w.value === parseInt(product.warranty))) {
          this.warranties.push({name: product.warranty, value: parseInt(product.warranty)})
        }
      }
    ...
    sortRows: function(sortKey) {clearTimeout(this._timerId)
