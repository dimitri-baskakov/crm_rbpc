import {randString, arrayRemove, hasClass, getOffset, binarySearch} from 'helper-js';
...
var targets = {
  ...
  'after': info => {
    info.store.$emit('targets-info', { id: info.targetNode.id, parent_id: info.dplh.parent.id, type: 'after' });
    clearTimeout(timeout)
    insertDplhAfterTo(info.dplh, info.targetNode);
  },
  'before': info => {
    info.store.$emit('targets-info', { id: info.targetNode.id, parent_id: info.dplh.parent.id, type: 'before' });
    clearTimeout(timeout)
  ...
  'append': info => {
    info.store.$emit('targets-info', { id: info.targetNode.id, parent_id: info.dplh.parent.id, type: 'append' });
    clearTimeout(timeout)
  ...
  'prepend': info => {
    info.store.$emit('targets-info', { id: info.targetNode.id, parent_id: info.dplh.parent.id, type: 'prepend' });
    clearTimeout(timeout)
    if (isNodeDroppable(info.targetNode)) {
      timeout = setTimeout(function () {
  ...
  'after target parent': info => {
    info.store.$emit('targets-info', {id: info.targetNode.id, parent_id: info.dplh.parent.id, type: 'after_target_parent'});
    clearTimeout(timeout)
    insertDplhAfterTo(info.dplh, info.targetNode.parent);
  },
  // append to prev sibling
  'append prev': info => {
    info.store.$emit('targets-info', { id: info.targetNode.id, parent_id: info.dplh.parent.id, type: 'append_prev' });
    clearTimeout(timeout)
  ...
  'append current tree': info => {
    info.store.$emit('targets-info', {id: info.targetNode.id, parent_id: info.dplh.parent.id, type: 'append_current_tree'});
    clearTimeout(timeout)
